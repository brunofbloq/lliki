...
TASK_BACKUP_IGNORE_ENTRY = "/wiki/tasks/.backup/"
TASK_BACKUP_IGNORE_COMMENT = "# Lliki generated task backups"
...
def _normalized_lines(text: str) -> list[str]:
    ...
    for line in lines:
        stripped = line.strip()
        if stripped in {SCRATCHPAD_IGNORE_COMMENT, TASK_BACKUP_IGNORE_COMMENT}:
            skip_blank_after_comment = False
            continue
        if stripped in {SCRATCHPAD_IGNORE_ENTRY, LEGACY_SCRATCHPAD_IGNORE_ENTRY, TASK_BACKUP_IGNORE_ENTRY}:
            skip_blank_after_comment = False
            continue
...
def ensure_scratchpad_ignored(root: Path, *, dry_run: bool = False) -> GitignoreResult:
    ...
    if path.exists():
        ...
        if (
            ...
            and stripped_lines.count(TASK_BACKUP_IGNORE_ENTRY) == 1
            and stripped_lines.count(SCRATCHPAD_IGNORE_COMMENT) == 1
            and stripped_lines.count(TASK_BACKUP_IGNORE_COMMENT) == 1
        ):
        ...
        if lines:
            lines.extend([
                "",
                SCRATCHPAD_IGNORE_COMMENT,
                SCRATCHPAD_IGNORE_ENTRY,
                TASK_BACKUP_IGNORE_COMMENT,
                TASK_BACKUP_IGNORE_ENTRY,
            ])
        else:
            lines.extend([
                SCRATCHPAD_IGNORE_COMMENT,
    ...
    content = (
        f"{SCRATCHPAD_IGNORE_COMMENT}\n{SCRATCHPAD_IGNORE_ENTRY}\n"
        f"{TASK_BACKUP_IGNORE_COMMENT}\n{TASK_BACKUP_IGNORE_ENTRY}\n"
    )
    if not dry_run:
...
def has_task_backup_ignore(root: Path) -> bool:
    path = root / ".gitignore"
    if not path.exists():
        return False
    try:
        return any(line.strip() == TASK_BACKUP_IGNORE_ENTRY for line in path.read_text(encoding="utf-8").splitlines())
    except OSError:
        return False
